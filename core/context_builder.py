...
class ContextBuilder:
    def __init__(self, settings: Settings, soul: Soul) -> None:
        ...
        self.episodic_repo = EpisodicMemoryRepository(settings.episodic_memory_file)
    ...
    def _memory_context(self, query: str) -> list[str]:
        episodic = self.episodic_repo.search(query, limit=self.settings.memory_retrieval_k)
        if episodic:
            return format_memory_blocks(episodic[:5])

        fallback = db.search_memories(self.settings.database_url, query=query, limit=self.settings.memory_retrieval_k)
        return [f"[memory:manual] {item['label']}: {item['content']}" for item in fallback[:5]]
